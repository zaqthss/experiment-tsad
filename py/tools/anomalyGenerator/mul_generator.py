...
class MulDataGenerator:
    ...
    def generate_timeseries(self):
        self.data = np.empty(shape=[0, self.STREAM_LENGTH], dtype=float)
    ...
    def set_timeseries(self, df, timestamp):
        self.STREAM_LENGTH = df.shape[0]
        self.data = np.empty(shape=[0, self.STREAM_LENGTH], dtype=float)
        for i in range(self.dim):
            self.behavior_config[i]['length'] = self.STREAM_LENGTH
            self.data = np.append(self.data, [df.iloc[:, i].to_numpy().flatten()], axis=0)
        self.data_origin = self.data.copy()
        self.label = np.zeros(self.STREAM_LENGTH, dtype=int)
...
def mul_example():
    ...
    multivariate_data = MulDataGenerator(dim=5, stream_length=400, behavior=BEHAVIOR, behavior_config=BEHAVIOR_CONFIG)
    multivariate_data.generate_timeseries()
    ...
    multivariate_data.collective_global_outliers(dim_no=2, ratio=0.05, radius=5, option='square', coef=1.5,
                                                 noise_amp=0.03, level=20, freq=0.04, offset=0.0)
...
def mul_non_cor(size):
    ...
    multivariate_data = MulDataGenerator(dim=2, stream_length=size, behavior=BEHAVIOR, behavior_config=BEHAVIOR_CONFIG)
    multivariate_data.generate_timeseries()
    ...
    multivariate_data.collective_global_outliers(dim_no=0, ratio=ratio / 2, radius=radius, option='square', coef=1.5,
                                                 noise_amp=0.03, level=20, freq=0.04, offset=0.0)
    multivariate_data.collective_seasonal_outliers(dim_no=1, ratio=ratio / 2, factor=3, radius=radius)
    timestamp = np.arange(1, size + 1)
    outlier_idxs = np.where(multivariate_data.label == 1)[0]
    # print(outlier_idxs)
    # outliers = list(multivariate_data.data[outlier_idxs])

    df = pd.DataFrame({'timestamp': timestamp,
                       'col_0': multivariate_data.data[0],
                       'col_1': multivariate_data.data[1],
                       'Label': multivariate_data.label})

    df.to_csv("../../dataset/mul_{type}_len_{length}_num_{num}_{size}_{rate}_{seed}.csv".format(
        type="subs", length=radius * 2, num=len(outlier_idxs), size=size, rate=round(ratio, 2), seed=seed), index=False)
...
def add_tao_mul_point(seed=1, size=10000):
    seed = 1
    np.random.seed(seed)
    ...
    df = pd.read_csv("../../dataset/input/TAO.csv", header=None)
    used_df = df.iloc[:size, :]
    timestamp = np.arange(1, size + 1)
    dim = used_df.shape[1]  # 3

    generator = MulDataGenerator(dim=dim, stream_length=size, behavior=BEHAVIOR, behavior_config=BEHAVIOR_CONFIG)
    generator.set_timeseries(used_df, timestamp)

    ratio = 0.1
    factors = [1.5, 1.5, 1.5]
    # radius = int(size / 100)
    radius = [int(size / 100), 50, 50]

    # generator.point_contextual_outliers(dim_no=0, ratio=ratio / dim, factor=factors[0], radius=radius[0])
    generator.point_contextual_outliers(dim_no=1, ratio=ratio / 2, factor=factors[1], radius=radius[1])
    generator.point_contextual_outliers(dim_no=2, ratio=ratio / 2, factor=factors[2], radius=radius[2])
    dim0_out_idxs = np.where(generator.data[0] == -9.99)[0]
    # print(dim0_out_idxs)
    generator.label[dim0_out_idxs] = 1
    dim1_out_idxs = np.where(generator.data[1] == -9.99)[0]
    generator.label[dim1_out_idxs] = 1
    dim2_out_idxs = np.where(generator.data[2] == -9.999)[0]
    generator.label[dim2_out_idxs] = 1

    df = pd.DataFrame({'timestamp': timestamp,
                       'col_0': generator.data[0],
                       'col_1': generator.data[1],
                       'col_2': generator.data[2],
                       'anomaly': generator.label})
    df.to_csv("../../dataset/tao_pointc_{size}_{rate}_{seed}.csv".format(
        size=size, rate=round(ratio, 2), seed=seed), index=False)

    plt.figure(figsize=(10, 15))
    plt.subplot(511)
    plt.plot(generator.timestamp, generator.data[0])
    plt.subplot(512)
    plt.plot(generator.timestamp, generator.data[1])
    plt.subplot(513)
    plt.plot(generator.timestamp, generator.data[2])
...
if __name__ == '__main__':
    ...
    seed = 1
    size = 100000
    # add_tao_mul_point(seed, size)
    mul_non_cor(size=5000)
