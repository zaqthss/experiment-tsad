...
def convert_to_windows(data, model):
    ...
    for i, g in enumerate(data):
        ...
        windows.append(w if 'TranAD' in model.name or 'Attention' in model.name else w.view(-1))
...
def bf_search(score, label, start, end=None, step_num=1, display_freq=1, verbose=False, direction='>'):
    """
    ...
    for i in range(search_step):
        ...
        preds = np.zeros(score.shape[0]).astype(bool)
        for i in range(score.shape[1]):
            l = score[:, i]
            lmin, lmax = np.min(l), np.max(l)
            l = (l - lmin) / (lmax - lmin)
            pred = eval('l{}threshold'.format(direction))
            preds = preds + pred
        target = calc_seq(score, label[:, 0], threshold, pred=preds, calc_latency=False)
        if target[0] > m[0]:
            m_t = threshold
            m = target
