...
class GDNWithPOT(MachineLearningAlgorithm):
    def __init__(self):
        super(GDNWithPOT, self).__init__()
    ...
    def init(self, args, series: timeSeries, trainingSeries: timeSeries, validSeries=None):
        ...
        if self.model.name in ['Attention', 'DAGMM', 'USAD', 'MSCRED', 'CAE_M', 'GDN', 'MTAD_GAT',
            ...
            self.trainD, self.testD = convert_to_windows(self.trainD, self.model), convert_to_windows(self.testD,self.model)
    ...
    def changeData(self ,series: timeSeries, validSeries: timeSeries=None):
