    ...
    public void dumpTuneResults(String folderDir, double alpha, POS_BIAS bias) throws IOException {
        ...
        for (String rawName : fileMap.keySet()) {
            ...
            if (meta.getSets().contains(dsName)) {
                TreeMap<Long, TimePointMulDim> anomalyPointMap = DataHandler.findAnomalyPoint(ts);
                TreeSet<Long> anomalyPoints = new TreeSet<>(anomalyPointMap.keySet());
                realAnomalyMapPoint.put(rawName, anomalyPoints);
                dsType = "point";
            } else {
