...
public class SAND implements UniDimAlgorithm {
  ...
  public void run() {
    ...
    ArrayList<Double> score_original = compute_score();
    ArrayList<Double> score = new ArrayList<>();
    score.addAll(score_original);
    ArrayList<Score> all_score = new ArrayList<Score>();
    int len=score_original.size();
    for(int i=len;i<batch_size;i++){
      score.add(score_original.get(len-1));
    }
    joinall.addAll(score);
    int batch = 0;
    while (current_time < ts.size()&&current_time<Math.min(current_time + batch_size, ts.size() - subsequence_length)) {
      ...
      run_next_batch();
      score_original = compute_score();
      score.addAll(score_original);
      len=score_original.size();
      for(int i=len;i<batch_size;i++){
        score.add(score_original.get(len-1));
      }
    ...
    this.score=joinall;
    for (int i = 0; i < score_original.size(); i++) {
      Score s = new Score(i, score_original.get(i));
      all_score.add(s);
    }
    ...
    ArrayList<Double> score = compute_score();
    ArrayList<Score> all_score = new ArrayList<Score>();
    ArrayList<Double> joinall = new ArrayList<>();
    joinall.addAll(score);
    for(int i=score.size();i<batch_size;i++){
      joinall.add(score.get(score.size()-1));
    }
