...
public class LRRDS implements MultiDimAlgorithm {
  ...
  private void outlier() {
    ...
    ArrayList<Double> join_all=new ArrayList<>();
    for(int i=0;i<(int)cp_profile[0][2];i++){
      join_all.add(deg.get(0));
    }
    for(int i=0;i<cp_profile.length;i++){
        for(int j=(int)cp_profile[i][2];j<(int)cp_profile[i][3];j++){
        join_all.add(deg.get(i));
      }
    }
    for(int i=(int)cp_profile[cp_profile.length-1][3];i< MulT.getLength();i++){
      join_all.add(deg.get(deg.size()-1));
    }
    join_all=MaxMinScaler(join_all);
    this.score=join_all;
