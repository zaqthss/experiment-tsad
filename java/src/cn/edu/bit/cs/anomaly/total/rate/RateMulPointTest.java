    ...
    for (int index = 0; index < VARSIZE; ++index) {
      ...
      if (willOperate[algIndex]) {
        for (int seed : seeds) {
          ...
          cpod = new CPOD();
          Map<String, Object> cpodParams = meta.getDataAlgParam().get(dsName)
          ...
          fh.writeMiddleResult(
              "..\\Middle\\Rate\\" + String.join("_", algNames[algIndex], dsName, anomalyType, size,
                  vars[index], seed + ""), timeSeriesMulDim);
          DataHandler.evaluate(timeSeriesMulDim, realAnomalyMulMap.get(seed),
      ...
      if (willOperate[algIndex]) {
        for (int seed : seeds) {
          ...
          algtime[algIndex][1] = System.currentTimeMillis();
          totaltime[index][algIndex] += algtime[algIndex][1] - algtime[algIndex][0];
          fh.writeMiddleResult("..\\Middle\\Rate\\"+String.join("_",algNames[algIndex],dsName,anomalyType,size,vars[index],seed+""),timeSeriesMulDim);
      ...
      // write results
      fh.writeResults("rate", "mul-point-" + dsName + "-" + anomalyType, vars, algNames,
          metricNames, totaltime,
          metrics, seeds.size());
    } // end of rIndex
  }
}