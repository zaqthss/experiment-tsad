...
public class SPLengthTest {
    ...
    public static void main(String[] args) throws ArgumentParserException {
        ...
        boolean[] willOperate = {true, true, true, true, true, false, false, false, false};

        String[] algNames = {"PBAD", "SAND", "NP", "MERLIN", "IDK", "CPOD", "NETS", "Stare", "SHESD"};
        ...
        for (int index = 0; index < VARSIZE; ++index) {
            ...
            if (willOperate[algIndex]) {
                for (int seed : seeds) {
                    ...
                    idkParams.put("window_size", Integer.parseInt(vars[index]));
          ...
          /*  algIndex++;
            if (willOperate[algIndex]) {
                for (int seed : seeds) {
                    ...
                    predictAnomaly = DataHandler.transPointToRange(new TreeSet<>(predictPoint.keySet()));
                    String dumpPath = String.format(
                            "%s/%s_%s_%s_l_%s.csv", "../middle/test", algNames[algIndex], filePrefix, vars[index], "sub");
                    fh.writeAnomalyRange(predictAnomaly, dumpPath);
                    DataHandler.evaluate(
                            alpha, bias, predictAnomaly, realAnomalyMap.get(seed), metrics[index][algIndex]);
                }
            ...
            // write results
           // fh.writeResults("length", "length-sub1-" + dsName, vars, algNames,
