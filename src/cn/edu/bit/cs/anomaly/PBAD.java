...
public class PBAD implements MultiDimAlgorithm {
  ...
  public void run() {
    ...
    for (int i = 0; i < score.size(); i++) {
      SubseqScore s = new SubseqScore(i * window_incr * bin_size, score.get(i));
      all_score.add(s);
    }
    ...
    for (int i = 0; i < Math.min(top_k, all_score.size()); i++) {
      ...
      for (int j = index; j < Math.min(window_size * bin_size + index, MulT.getLength()); j++) {
        TimePointMulDim tpm = (TimePointMulDim) MulT.getTimeseries().get(j);
        tpm.setIs_anomaly(IS_ANOMALY.TRUE);
        // MulT.getTimePoint(j).setIs_anomaly(IS_ANOMALY.TRUE);
      }
    }
  }
