...
/** Test accuracy on univariate point anomaly TODO: change vars, algNames, add algorithms */
public class UniPointTest {
...
//    String[] vars = {"YahooA1", "stock_10k_point", "uni_point"};
    ...
    for (int index = 0; index < VARSIZE; ++index) {
      ...
      if (willOperate[algIndex]) {
        ...
        luminol.run();
        algtime[algIndex][1] = System.currentTimeMillis();
        DataHandler.evaluate(timeseries, realAnomalyMap.get(dsName), metrics[index][algIndex]);
      }
      // SHESD
      algIndex++;
      if (willOperate[algIndex]) {
