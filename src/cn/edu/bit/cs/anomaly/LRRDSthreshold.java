...
public class LRRDSthreshold implements MultiDimAlgorithm {
  ...
  public void run() {
    fit();
    if (success == true) {
      top_k=(int) Math.ceil(threshold*all_score.size());
      for (int i = 0; i < Math.min(top_k, all_score.size()); i++) {
        ...
        for (int j = (int) record[2]; j <= Math.min((int) record[3],MulT.getLength()); j++) {
