...
public class NETS implements  MultiDimAlgorithm{
	public double R;
	public int K;
	public int dim;
	public int subDim;
	...
	public NETSStreamGenerator streamGenerator;
	public TimeSeriesMulDim orign;
	...
	public void run() {
		...
		int itr = 0;
		do {
			newSlideTuples = streamGenerator.getNewSlideTuples(itr, S);
			if(newSlideTuples.size()<1)
				break;
			calcNetChange(newSlideTuples, itr);	/* Calculate net effect*/
			findOutlier("NETS", itr);		/* Find outliers */

			// Save CPU time & Peak memory
			if(itr>=nS-1) {
				for(Tuple t :outliers){
					this.orign.getTimePoint(t.id).setIs_anomaly(IS_ANOMALY.TRUE);
				}
			}
			itr++;
		} while(itr <nW+nS-1);
	...
	public void init(Map<String, Object> args, TimeSeriesMulDim timeseries) {
		timeseries.clear();
		this.orign = timeseries;
		this.dim = timeseries.getDim();
		this.subDim = (int) args.get("subDim");
		this.subDimFlag = dim != subDim;
		this.R = (double) args.get("R");
		this.K = (int) args.get("K");
		this.S = (int) args.get("S");
		this.W = (int) args.get("W");
		this.nW = (int) args.get("nW");
		this.nS = W/S;
		this.neighCellIdxDist = Math.sqrt(subDim)*2;
		this.neighCellFullDimIdxDist = Math.sqrt(dim)*2;
		this.streamGenerator = new NETSStreamGenerator(timeseries);
		this.maxValues = streamGenerator.getMaxValues();
		this.minValues = streamGenerator.getMinValues();
